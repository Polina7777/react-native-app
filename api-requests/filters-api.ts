...
export const filtersByFiltersForm = async (filters)=>{
    const response = await fetch(`${url_ngrok}api/foods?populate=*&filters[extra_info][kcal][$containsi]=${filters.kcal}&filters[extra_info][serve][$containsi]=${filters.serve}&filters[extra_info][grams][$containsi]=${filters.grams}`,{method:'GET'});
    const data = await response.json();
    const filteredData = data.data
    return filteredData;
}
